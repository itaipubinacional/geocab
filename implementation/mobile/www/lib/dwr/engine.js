...
(function() {
	...
	dwr.engine.setActiveReverseAjax = function(activeReverseAjax) {
		if (activeReverseAjax) {
			...
			if (!dwr.engine._retryIntervals
					|| dwr.engine._retryIntervals.length === 0) {
				dwr.engine._retryIntervals = dwr.engine._defaultRetryIntervals;
			}
			dwr.engine._activeReverseAjax = true;
			if (dwr.engine._initializing)
				return;
			dwr.engine._poll();
		} else {
	...
	dwr.engine.setNotifyServerOnPageUnload = function(notify, asyncUnload) {
		dwr.engine._asyncUnload = (asyncUnload !== undefined) ? asyncUnload
				: false;
	...
	dwr.engine.setNotifyServerOnPageLoad = function(notify) {
		...
		if (notify && !dwr.engine._initializing
				&& !dwr.engine._isNotifyServerOnPageLoadSent) {
	...
	dwr.engine._pathToDwrServlet = "/broker";
	...
	dwr.engine._initializer = {
		...
		preInit : function() {
			...
			dwr.engine.util.addEventListener(window, 'beforeunload',
					dwr.engine._beforeUnloader);
			dwr.engine.util.addEventListener(window, 'unload',
		...
		loadDwrConfig : function() {
			if (typeof dwrConfig != "undefined") {
				for (p in dwrConfig) {
					var methodName = "set" + p.charAt(0).toUpperCase()
							+ p.substring(1);
	...
	dwr.engine._handleError = function(batch, ex) {
		...
		if (batch && batch.isPoll) {
			...
			ignoreIfUnloading(batch, function() {
				if (batch) {
					var textHtmlHandler = batch.textHtmlHandler
							|| batch.globalTextHtmlHandler;
					if (ex.name === "dwr.engine.textHtmlReply"
							&& textHtmlHandler) {
						dwr.engine.util.logHandlerEx(function() {
							textHtmlHandler(ex);
						});
				...
				if (batch) {
					dwr.engine.util.logHandlerEx(function() {
						if (typeof batch.errorHandler == "function")
							...
							if (typeof batch.globalErrorHandler == "function")
	...
	dwr.engine._handlePollRetry = function(batch, ex) {
		...
		if (dwr.engine._retries === dwr.engine._retryIntervals.length - 1) {
			dwr.engine._debug("poll retry - going offline: " + retryInterval
					/ 1000 + " seconds");
			...
			var heartbeatInterval = setInterval(
					function() {
						if (dwr.engine._maxRetries === -1
							...
							dwr.engine._execute(null, '__System',
							...
							dwr.engine._retries++;
							dwr.engine
									._debug("DWR Offline - poll retry - interval: "
							...
							dwr.engine
									._handlePollStatusChange(false, ex, batch);
						}
					}, retryInterval);
		} else {
			dwr.engine._retries++;
			dwr.engine.batch.remove(batch);
			dwr.engine._debug("DWR Online - poll retry - interval: "
					+ retryInterval / 1000 + " seconds");
	...
	dwr.engine._handlePollStatusChange = function(newStatus, ex, batch) {
		if (batch.isPoll || dwr.engine._isHeartbeatBatch(batch)) {
			...
			if ((changed || maxRetriesReached)
					&& typeof dwr.engine._pollStatusHandler === "function")
				dwr.engine.util.logHandlerEx(function() {
	...
	dwr.engine._getObject = function(prop) {
		...
		while (parts.length > 0) {
			...
			if (value && value.tagName
					&& document.getElementById(currprop) == value)
				return undefined;
			scope = value;
		}
		return value;
	...
	dwr.engine._setObject = function(prop, obj) {
		...
		while (parts.length > 0) {
			...
			if (parts.length === 0) {
				...
				if (level == null || level.tagName
						&& document.getElementById(currprop) == level) {
	...
	dwr.engine.remote = {
					...
					if (typeof handlers.callback == "function") {
						dwr.engine.util.logHandlerEx(function() {
							if (handlers.callbackArg !== undefined)
								handlers.callback.call(handlers.callbackScope,
		...
		handleReverseAjax : function(reverseAjaxIndex, reverseAjaxFunc) {
			if (reverseAjaxIndex < dwr.engine._nextReverseAjaxIndex)
				return;
	...
	dwr.engine.serialize = {
		...
		convert : function(batch, directrefmap, otherrefmap, data, name, depth) {
			...
			case "object":
				...
				if (data.$dwrByRef)
					batch.map[name] = dwr.engine.serialize.convertByReference(
					...
					batch.map[name] = dwr.engine.serialize.convertArray(batch,
							directrefmap, otherrefmap, data, name, depth + 1);
				else if (data && data.tagName
						&& data.tagName.toLowerCase() == "input" && data.type
					...
					if (data.nodeName && data.nodeType) {
						...
						batch.map[name] = dwr.engine.serialize.convertObject(
								batch, directrefmap, otherrefmap, data, name,
								depth + 1);
					}
				}
		...
		convertObject : function(batch, directrefmap, otherrefmap, data, name,
			...
			var elementset = (data.constructor
					&& data.constructor.$dwrClassMembers ? data.constructor.$dwrClassMembers
					: data);
			var element;
		...
		convertDom : function(data) {
			return "xml:"
					+ encodeURIComponent(dwr.engine.serialize
							.serializeDom(data));
		},
		serializeDom : function(data) {
			...
			if (window.XMLSerializer)
				...
				throw new Error(
						"The browser doesn't support XML serialization of: "
								+ data);
			return output;
		},
	...
	dwr.engine.transport = {
		...
		send2 : function(batch) {
			...
			} else if (batch.isPoll && dwr.engine._useStreamingPoll == "true"
					&& dwr.engine.util.ieCondition("if (IE 8)|(IE 9)")) {
				batch.transport = dwr.engine.transport.iframe;
			} else {
				batch.transport = dwr.engine.transport.xhr;
			}
		...
		xhr : {
			...
			send : function(batch) {
				...
				batch.mode = batch.isPoll ? dwr.engine._ModePlainPoll
		...
		iframe : {
			...
			send : function(batch) {
				...
				if (window.location.hostname != document.domain)
					batch.map.documentDomain = document.domain;
				var idname = dwr.engine.transport.iframe.getId(batch);
				batch.div1 = document.createElement("div");
				batch.div1.innerHTML = "<iframe frameborder='0' style='width:0px;height:0px;border:0;display:none;' id='"
				...
				dwr.engine.util
						.addEventListener(
								...
								function(ev) {
									...
									if (!batch.loadingStarted)
										return;

									while (batch.fileInputs.length > 0) {
										var entry = batch.fileInputs.pop();
										entry.original.setAttribute("id",
												entry.clone.getAttribute("id"));
										entry.original.setAttribute("name",
												entry.clone
														.getAttribute("name"));
										entry.original.style.display = entry.clone.style.display;
										entry.clone.parentNode.replaceChild(
									...
									if (dwr.engine._queuedBatchException) {
										dwr.engine
												._handleError(
														batch,
														dwr.engine._queuedBatchException);
										dwr.engine._queuedBatchException = null;
										return;
									}

									try {
										var contentDoc = batch.iframe.contentDocument
												|| batch.iframe.contentWindow.document;
									} catch (ex) {
										var contentEx = ex;
									}
									if (typeof dwr != "undefined") {
										var htmlResponse = contentDoc
												&& contentDoc.documentElement ? contentDoc.documentElement.outerHTML
												: "(Could not extract HTML response: "
														+ contentEx.message
														+ ")";
										dwr.engine
												._handleError(
			...
			getId : function(batch) {
				return batch.isPoll ? "dwr-if-poll-" + dwr.engine._instanceId
						+ "-" + batch.map.batchId : "dwr-if-"
						+ dwr.engine._instanceId + "-" + batch.map.batchId;
			},
			...
			beginLoader : function(batch, idname) {
				...
				batch.mode = batch.isPoll ? dwr.engine._ModeHtmlPoll
			...
			remote : {
				...
				endChunk : function(iframeWindow) {
					setTimeout(function() {
						...
						var scriptTags = iframeWindow.document.body
	...
	dwr.engine.batch = {
		...
		create : function() {
			...
			if (!dwr.engine._activeReverseAjax)
				batch.map.nextReverseAjaxIndex = dwr.engine._nextReverseAjaxIndex;
		...
		createPoll : function() {
			var batch = {
				...
				map : {
					callCount : 1,
					nextReverseAjaxIndex : dwr.engine._nextReverseAjaxIndex
				},
				paramCount : 0,
				preHooks : [],
				postHooks : [],
...
batch.handlers[batch.map.callCount] = {
		...
		merge : function(batch, overrides) {
			...
			for (var i = 0; i < dwr.engine._propnames.length; i++) {
				...
				if (dwr.engine._singleShot && propname == "errorHandler")
					continue;
		...
		remove : function(batch) {
			...
			if (batch == dwr.engine._batch)
				dwr.engine._batch = null;
			if (batch == dwr.engine._pollBatch)
				dwr.engine._pollBatch = null;
	...
	dwr.engine.util = {
		...
		ieCondition : function(cond) {
			if (!(cond in dwr.engine._ieConditions)) {
				...
				dwr.engine._ieConditions[cond] = !!(div
		...
		isCallOptionArgument : function(lastArg) {
			return (typeof lastArg === "object" && (typeof lastArg.callback === "function"
					...
					|| typeof lastArg.errorHandler === "function"
					|| typeof lastArg.warningHandler === "function" || lastArg
					.hasOwnProperty("async")));
		},
		...
		logHandlerEx : function(func) {
			try {
				...
				dwr.engine
						._debug("Exception occured in user-specified handler:");
	...
	dwr.hub = {
		...
		subscribe : function(topicName, callback, scope, subscriberData) {
			...
			dwr.engine._execute(null, '__System', 'subscribe', topicName,
					subscription, {});
			return subscription;
		},
	...
	dwr.data.Cache.prototype.viewRegion = function(region, callbackObj) {
		...
		return dwr.engine._execute(null, '__Data', 'viewRegion', [
				this.storeId, region, this.listener, callbackObj ]);
	};

	dwr.data.Cache.prototype.viewItem = function(itemId, callbackObj) {
		return dwr.engine._execute(null, '__Data', 'viewItem', [ this.storeId,
				itemId, this.listener, callbackObj ]);
	};

	dwr.data.Cache.prototype.unsubscribe = function(callbackObj) {
		if (this.listener) {
			return dwr.engine._execute(null, '__Data', 'unsubscribe', [
					this.storeId, this.listener, callbackObj ]);
		}
	};

	dwr.data.Cache.prototype.update = function(items, callbackObj) {
		return dwr.engine._execute(null, '__Data', 'update', [ this.storeId,
				items, callbackObj ]);
	};
