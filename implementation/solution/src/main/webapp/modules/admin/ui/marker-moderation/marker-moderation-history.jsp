	...
	<div class="content-tab" style="width:35%; height: 100%; float:left; margin: 20px;" overflow="auto">
		...
		<div ng-repeat="markerModeration in markersModeration" >
			...
			<div class="{{markerModeration.status == 'PENDING' ? 'alert warning' : markerModeration.status == 'ACCEPTED' ? 'alert success' : 'alert danger'}}" style="text-align:center;height:75px">
				...
				<span ng-if="markerModeration.status == 'REFUSED'"><i style="float:right;cursor:pointer;left:64px;top:40px" class="glyphicon glyphicon-chevron-up" ng-click="showFields(false)" ng-if="!hiding"></i></span>
				<span ng-if="markerModeration.status == 'REFUSED'"><i style="float:right;cursor:pointer;left:75px;top:40px" class="glyphicon glyphicon-chevron-down" ng-click="showFields(true)" ng-if="hiding"></i></span>
			</div>
