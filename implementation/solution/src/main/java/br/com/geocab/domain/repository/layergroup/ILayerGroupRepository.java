...
public interface ILayerGroupRepository extends IDataRepository<LayerGroup, Long>
{
	 ...
	 */	
	 ...
	 @Query(value="SELECT New LayerGroup ( layerGroup.id, layerGroup.name, layerGroup.orderLayerGroup)"
			 + "FROM LayerGroup layerGroup "
	...
	@Query(value="SELECT New Layer (layer.id,layer.name, layer.orderLayer, layer.icon)"
			+ " FROM Layer layer "
